...
import os
#* ====================输入文件====================
#* 输入文件:/0-required_files/input_denovo/*中的所有变异数据
#* ===============================================
#* ====================输出文件====================
#* /output/denovo/all_denovo.txt，包含所有 de novo 突变及其样本信息
#* ====================输出文件格式================
# denovo	sample	sample_denovo_count
# T12586C	F661-PMID31123110-germline	2
# T13819C	F1131-PMID31123110-germline	1
# A1409G	F138-PMID31123110-germline	2
#* ===============================================
#* 这一步集合了不同来源的线粒体新发突变，为后续统计提供最大样本量。
...
def extract_denovo(file: TextIO):
    ...
    for row in csv.reader(open('0-required_files/input_denovo/germline/PMID31123110_Data_S1.txt'), delimiter='\t'):
        if row[6] == "de novo":
    ...
    for row in csv.reader(open('0-required_files/input_denovo/germline/PMID25313049_TableS3.txt'), delimiter='\t'):
    ...
    for row in csv.reader(open('0-required_files/input_denovo/germline/PMID31757848_TableS3.txt'), delimiter='\t'):
    ...
    for row in csv.reader(open('0-required_files/input_denovo/germline/PMID26916109_Dataset_S1_Heteroplasmy_list.txt'),
                          delimiter='\t'):
    ...
    # 数据集5 - 自有 SPARK 分析结果
    # 由于数据限制无法公开
    if os.path.exists('0-required_files/input_denovo/germline/SPARK_mtDNA_de_novo.txt'):
        for row in csv.DictReader(open('0-required_files/input_denovo/germline/SPARK_mtDNA_de_novo.txt'), delimiter='\t'):
    ...
    for row in csv.reader(open('0-required_files/input_denovo/somatic_tissue/PMID25313049_TableS3.txt'), delimiter='\t'):
    ...
    for row in csv.reader(open('0-required_files/input_denovo/somatic_tissue/PMID31757848_TableS4.txt'), delimiter='\t'):
    ...
    for row in csv.reader(
            open('0-required_files/input_denovo/somatic_tissue/PMID30827679_NIHMS1518665-supplement-10.txt'),
            delimiter='\t'):
    ...
    for row in csv.DictReader(open('0-required_files/input_denovo/somatic_cancer/TCMA-MutationSNV.tsv'), delimiter='\t'):
