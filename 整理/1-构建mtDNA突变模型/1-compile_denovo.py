...
"""
此脚本汇总多个公开文献及自有数据中的线粒体 DNA（mtDNA）de novo 突变数据
统一格式并输出一个总表，用于下游分析。它会读取不同来源的 germline（生殖系）、
somatic tissue（体细胞组织）和 somatic cancer（体细胞癌）数据文件，解析出
每个样本的 de novo 突变及计数，然后写入一个统一输出文件。
"""
...
def rcrs_pos_to_ref():
    ...
    """
    dictionary = {}
    for row in csv.DictReader(open('../2-必需文件/synthetic_vcf/NC_012920.1_synthetic_vep_noheader.vcf'),
                              delimiter='\t'):
...
def extract_denovo(file: TextIO):
    ...
    for row in csv.reader(open('../2-必需文件/input_denovo/germline/PMID31123110_Data_S1.txt'), delimiter='\t'):
        if row[6] == "de novo":
    ...
    for row in csv.reader(open('../2-必需文件/input_denovo/germline/PMID25313049_TableS3.txt'), delimiter='\t'):
    ...
    for row in csv.reader(open('../2-必需文件/input_denovo/germline/PMID31757848_TableS3.txt'), delimiter='\t'):
    ...
    for row in csv.reader(open('../2-必需文件/input_denovo/germline/PMID26916109_Dataset_S1_Heteroplasmy_list.txt'),
                          delimiter='\t'):
    ...
    # 数据集5 - 自有 SPARK 分析结果
    # 由于数据限制无法公开
    if os.path.exists('../2-必需文件/input_denovo/germline/SPARK_mtDNA_de_novo.txt'):
        for row in csv.DictReader(open('../2-必需文件/input_denovo/germline/SPARK_mtDNA_de_novo.txt'), delimiter='\t'):
    ...
    for row in csv.reader(open('../2-必需文件/input_denovo/somatic_tissue/PMID25313049_TableS3.txt'), delimiter='\t'):
    ...
    for row in csv.reader(open('../2-必需文件/input_denovo/somatic_tissue/PMID31757848_TableS4.txt'), delimiter='\t'):
    ...
    for row in csv.reader(
            open('../2-必需文件/input_denovo/somatic_tissue/PMID30827679_NIHMS1518665-supplement-10.txt'),
            delimiter='\t'):
    ...
    for row in csv.DictReader(open('../2-必需文件/input_denovo/somatic_cancer/TCMA-MutationSNV.tsv'), delimiter='\t'):
