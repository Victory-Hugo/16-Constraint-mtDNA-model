...
# 导入带有破折号的模块
spec = importlib.util.spec_from_file_location("compile_denovo", "1-compile_denovo.py")
compile_denovo = importlib.util.module_from_spec(spec)
sys.modules["compile_denovo"] = compile_denovo
spec.loader.exec_module(compile_denovo)
...
def rcrs_pos_to_trinucleotide():
	...
	dict = {}
	for row in csv.DictReader(open('../2-必需文件/synthetic_vcf/NC_012920.1_synthetic_vep_noheader.vcf'), delimiter='\t'):
		pos = int(row["POS"])
		ref = row["REF"]
...
def gnomad_annotate():
	...
	for row in csv.DictReader(
			open('../2-必需文件/databases/gnomad.genomes.v3.1.sites.chrM.reduced_annotations.tsv'), delimiter='\t'):
		if row["filters"] == "PASS":
...
def phylop_annotate():
	...
	for row in open('../2-必需文件/insilicos/chrM.phyloP100way.wigFix'):
...
def vep_annotate():
	...
	for row in csv.DictReader(
			open("../2-必需文件/synthetic_vcf/NC_012920.1_synthetic_vep_splitvarstwogenes.vcf"), delimiter="\t"):
		# gene or locus
...
def tRNA_positions():
	...
	"""
	dict = {}
	for row in csv.DictReader(open('../2-必需文件/other_annotations/tRNA_positions.txt'), delimiter='\t'):
...
def RNA_domains_mods():
	...
	dict = {}
	for row in csv.DictReader(
			open('../2-必需文件/other_annotations/mito_RNA_modifications_and_domains.txt'), delimiter='\t'):
...
def RNA_base_type():
	...
	RNA_dict = {}
	for row in csv.DictReader(open('../2-必需文件/other_annotations/all_RNA_bases.tsv'), delimiter='\t'):
...
def uniprot_annotations():
	...
	dict = {}
	for row in csv.reader(open('../2-必需文件/other_annotations/uniprot_beds_chrMT_combined.txt'), delimiter='\t'):
...
def curated_func_sites():
	...
	dict = {}
	for row in csv.DictReader(
			open('../2-必需文件/other_annotations/CI_proton_residues_PMID32972993.txt'), delimiter='\t'):
		for pos in list(range(1, 16570)):
...
def apogee():
	...
	"""
	dict = {}
	for row in csv.DictReader(open('../2-必需文件/insilicos/MitImpact_db_3.0.6.txt'), delimiter='\t'):
...
def mitotip():
	...
	dict = {}
	for row in csv.DictReader(open('../2-必需文件/insilicos/mitotip_scores.txt'), delimiter='\t'):
		prediction = ''
		if row["Quartile"] == "Q1":
...
def hmtvar():
	...
	"""
	dict = {}
	for row in csv.DictReader(open("../2-必需文件/insilicos/hmtvar_annotations.txt"), delimiter="\t"):
		insilico = ""
...
def in_helix():
	...
	"""
	dict = {}
	for row in csv.DictReader(open('../2-必需文件/databases/HelixMTdb_20200327.tsv'), delimiter='\t'):
...
def mitomap():
	...
	"""
	dict1, dict2 = {}, {}
	for row in csv.DictReader(open('../2-必需文件/databases/MITOMAP_disease_2022-05-25.txt'), delimiter='\t'):
	...
	for row in csv.DictReader(open('../2-必需文件/databases/MITOMAP_polymorphisms_2022-07-14.txt'), delimiter='\t'):
		if (len(row["ref"]) == 1) and (len(row["alt"]) == 1) and row["alt"].isalpha() and (row["ref"] != row["alt"]):  # if SNVs
...
def clinvar():
	...
	"""
	dict = {}
	for row in csv.DictReader(open('../2-必需文件/databases/clinvar_result_chrMT_SNVonly_05252022.txt'), delimiter='\t'):
...
def chimp_ref_lookup():
	...
	dict = {}
	with open('../2-必需文件/other_annotations/human-shifted_chimp_mt_aln.txt') as file:
...
def annotate(input_file: str):
	...
	for row in csv.DictReader(open(input_file), delimiter='\t'):
		...
		in_phylo = 1 if ("\n" + variant + "\n") in open('../2-必需文件/databases/phylotree_variants.txt').read() else 0
		...
		RNA_bridge = "Yes" if ("\n" + row["POS"] + "\n") in open('../2-必需文件/other_annotations/rRNA_bridge_bases.txt').read() else "No"
